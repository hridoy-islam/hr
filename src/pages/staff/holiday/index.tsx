...
const holidayRequestSchema = z.object({
  ...
  reason: z
    .string({ required_error: 'Reason is required' }).optional(),
  ...
  startDate: z
    .date({ required_error: 'Please select a date range', invalid_type_error: 'Please select a date range' }),

  endDate: z
    .date({ required_error: 'Please select a date range', invalid_type_error: 'Please select a date range' }),

  totalDays: z
    .number({ required_error: 'Total days is required', invalid_type_error: 'Total days must be a number' })
    .min(0.5, 'Total days must be at least 0.5'),

  totalHours: z
    .number({ required_error: 'Total hours is required', invalid_type_error: 'Total hours must be a number' })
...
type HolidayFormErrors = Partial<Record<keyof z.infer<typeof holidayRequestSchema> | 'dateRange', string>>;
...
const HolidayPage: React.FC = () => {
  ...
  const handleDaysChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormErrors((prev) => ({ ...prev, totalDays: undefined, totalHours: undefined }));
  };
  ...
  const fetchHolidayAllowance = async () => {
    try {
      ...
      if (Array.isArray(responseData)) {
        holidayRecord = responseData.find((item: any) => item.year === selectedYear);
      ...
      if (holidayRecord) {
        ...
        setLeaveAllowance({
          holidayAllowance: 0, holidayAccured: 0, usedHours: 0, bookedHours: 0,
          requestedHours: 0, remainingHours: 0, unpaidLeaveTaken: 0, 
          unpaidBookedHours: 0, unpaidLeaveRequest: 0,
        });
      }
    } catch (err: any) {
  ...
  const fetchLeaveRequests = async () => {
    try {
      setError(null);
      const response = await axiosInstance.get(`/hr/leave?userId=${eid}&holidayYear=${selectedYear}&limit=all`);
  ...
  const generateLeaveDaysArray = (start: Date, end: Date, currentHolidayType: string) => {
    const daysArray   = [];
  ...
  const allowanceStatsList = useMemo(() => [
    { label: 'Opening This Year', value: leaveAllowance.holidayAllowance,   color: 'text-gray-800' },
  ...
  if (error) {
    return (
      <div className="p-8 text-center">
        ...
        <Button onClick={() => { fetchLeaveRequests(); fetchHolidayAllowance(); }}>
          Retry
        </Button>
      </div>
    );
                    ...
                    <SheetContent className="overflow-y-auto sm:max-w-[450px]">
                      ...
                      <div className="space-y-5">
                        ...
                        <div>
                          ...
                          <ShadcnSelect
                            value={selectedYear}
                            onValueChange={(val) => { setSelectedYear(val); clearError('holidayYear'); }}
                          >
                            <SelectTrigger id="holiday-year" className={formErrors.holidayYear ? 'border-red-500' : ''}>
                          ...
                          {formErrors.holidayYear && <p className="mt-1 text-xs text-red-500">{formErrors.holidayYear}</p>}
                        </div>
                        ...
                        <div>
                          ...
                          <ReactSelect
                            ...
                            onChange={(option) => { setSelectedType(option?.value || ''); clearError('holidayType'); }}
                            ...
                            styles={{
                              control: (base) => ({
                                ...
                                '&:hover': { borderColor: formErrors.holidayType ? '#ef4444' : base.borderColor },
                              }),
                            }}
                          />
                          {formErrors.holidayType && <p className="mt-1 text-xs text-red-500">{formErrors.holidayType}</p>}
                        </div>

                        {/* Date Range Picker */}
                        <div className="flex flex-col gap-1">
<Label>Leave Period (DD-MM-YYYY)</Label>                          <DatePicker
                            selectsRange
                            startDate={startDate}
                            endDate={endDate}
                            onChange={(dates) => {
                            ...
                            minDate={new Date()}
                          />
                          {formErrors.dateRange && <p className="mt-1 text-xs text-red-500">{formErrors.dateRange}</p>}
                        </div>

                        {/* Total Days */}
                        <div className="space-y-1">
                          <Label htmlFor="duration-days">Holiday Duration (Days)</Label>
                          <Input
                            id="duration-days"
                            type="number"
                            min="0"
                          ...
                          />
                          {formErrors.totalDays && <p className="text-xs text-red-500">{formErrors.totalDays}</p>}
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          />
                          {formErrors.totalHours && <p className="text-xs text-red-500">{formErrors.totalHours}</p>}
                        </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4 rounded-lg bg-blue-50 p-4 md:grid-cols-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-gray-800">{leaveAllowance.holidayAllowance.toFixed(1)} h</div>
                      <div className="text-sm text-gray-600">Total Allowance</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">{leaveAllowance.usedHours.toFixed(1)} h</div>
                      <div className="text-sm text-gray-600">Taken</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-orange-600">{leaveAllowance.bookedHours.toFixed(1)} h</div>
                      <div className="text-sm text-gray-600">Booked</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-purple-600">{leaveAllowance.remainingHours.toFixed(1)} h</div>
                        ...
                        {holidays.length > 0 ? (
                          holidays.map((holiday, index) => (
                            <TableRow key={`${holiday.startDate}-${holiday.title}-${index}`}>
                              ...
                              <TableCell className="font-medium w-[30%] whitespace-pre-wrap">{holiday?.reason || '-'}</TableCell>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  {allowanceStatsList.map(({ label, value, color }) => (
                    <div key={label} className="flex items-center justify-between border-b border-gray-300 py-2">
                      <span className="text-gray-600">{label}</span>
                      <span className={`font-semibold ${color}`}>{value.toFixed(1)} h</span>
                    </div>
                  ))}
