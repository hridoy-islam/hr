...
export default function StaffMeetingDetailsPage() {
  ...
  const [logToAcknowledge, setLogToAcknowledge] = useState<LogEntry | null>(
    null
  );
  ...
  return (
    <div className="min-h-screen space-y-4 rounded-md bg-white p-3 shadow-sm max-sm:pt-16 sm:space-y-6 sm:p-5">
      {/* Header Area */}
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:gap-2 sm:space-x-3">
          <h1 className="text-base font-semibold text-gray-900 sm:max-w-[65%] sm:text-lg">
            {meeting?.title || ''}
          </h1>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1 pb-0">
                        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                          <span className="whitespace-nowrap px-2 text-sm font-semibold text-black sm:px-2.5">
      ...
      <Dialog open={viewDocsDialogOpen} onOpenChange={setViewDocsDialogOpen}>
        <DialogContent className="w-[95vw] max-w-5xl rounded-xl border-gray-100 sm:w-full sm:rounded-xl">
          <DialogHeader className="border-b border-gray-100 pb-4">
            <DialogTitle className="flex items-start gap-2 text-base font-bold sm:items-center sm:text-xl">
              <FileText className="mt-0.5 h-5 w-5 flex-shrink-0 text-theme sm:mt-0" />
...
<div className={`grid gap-4 ${
  user.role === 'employee' 
    ...
    : ' lg:grid-cols-12'
              ...
              {user.role !== 'employee' && (
                <div className="space-y-4 border-t border-gray-100 pt-4 md:col-span-4 md:border-l md:border-t-0 md:pl-6 md:pt-0">
      ...
      >
        <DialogContent className="w-[95vw] max-w-xl rounded-xl border-gray-200 sm:w-full sm:rounded-xl">
          ...
          <DialogFooter className="gap-2 border-t border-gray-100 pt-4 sm:justify-end">
            ...
            <Button
              className="w-full min-w-[120px] bg-theme text-white hover:bg-theme/90 disabled:opacity-50 sm:w-auto"
