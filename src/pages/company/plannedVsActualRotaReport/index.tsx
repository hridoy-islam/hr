...
const calculateDurationMinutes = (
  ...
  if (leaveType) return 0; // Exclude leaveType from calculations
...
const formatDiff = (totalMinutes: number): string => {
  if (!totalMinutes || totalMinutes === 0) return '00:00';
...
interface DraggableRowProps {
  ...
  plannedRotaMap: Record<string, Record<string, any[]>>;
  employeeTotalDuration: Record<string, Record<string, number>>;
  employeePlannedDuration: Record<string, Record<string, number>>;
}
...
const DraggableRow: React.FC<DraggableRowProps> = ({
  ...
  const totalPlannedMins = employeePlannedDuration[departmentId]?.[user._id] || 0;
  const totalDiff = totalMins - totalPlannedMins;
  ...
  return (
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex flex-row items-center justify-between gap-4">
              ...
              <div className="flex-shrink-0 flex items-center gap-1   py-0.5 text-[10px] font-semibold tracking-wider ">
                
                {totalDiff !== 0 && (
                  <>Total Diff:
                  <span className={totalDiff > 0 ? 'text-theme' : 'text-red-500'}>
    ...
    {daysArray.map((day, idx) => {
        ...
        const dayPlannedRotas = (plannedRotaMap[user._id]?.[dateKey] || []).filter(
          (r: any) =>
            (typeof r.departmentId === 'object'
              ? r.departmentId._id
              : r.departmentId) === departmentId
        ...
        let actualDayMins = 0;
        dayRotas.forEach((r) => {
          // Exclude 'pending' actual rotas (treat as 0)
          if (r.status !== 'pending') {
            actualDayMins += calculateDurationMinutes(r.startTime, r.endTime, r.leaveType);
          }
        });

        let plannedDayMins = 0;
        dayPlannedRotas.forEach((r) => {
          // Exclude 'pending' planned rotas (treat as 0)
          if (r.status !== 'pending') {
            plannedDayMins += calculateDurationMinutes(r.startTime, r.endTime, r.leaveType);
          }
        });

        const diffMins = actualDayMins - plannedDayMins;
        ...
        return (
          ...
          >
            {diffMins !== 0 ? (
              <div className="flex flex-col gap-0.5 items-center justify-center min-h-[30px]">
                <div className={`text-[12px] font-bold ${diffMins > 0 ? 'text-theme' : 'text-red-500'}`}>
                  {formatDiff(diffMins)}
                </div>
              </div>
            ) : (
...
interface DraggableChildGroupProps {
  ...
  departmentPlannedDuration: Record<string, number>;
  moveRow: (
    departmentId: string,
    dragIndex: number,
    hoverIndex: number
...
const DraggableChildGroup: React.FC<DraggableChildGroupProps> = ({
  ...
  const deptPlannedMins = departmentPlannedDuration[childGroup.department._id] || 0;
  const deptDiff = deptTotalMins - deptPlannedMins;
    ...
    <>
      ...
      >
        <td className="sticky left-0 z-20 border-r border-gray-200 bg-inherit px-2 py-2 shadow-[2px_0_0_0_rgba(0,0,0,0.04)]">
          <div className="flex items-center justify-between pl-4">
            ...
            <div className="flex items-center gap-1  py-1 text-[11px] font-bold pl-12">
              {deptDiff !== 0 && (
                <> Total Diff:
                <span className={deptDiff > 0 ? 'text-theme' : 'text-red-500'}>
                  ({formatDiff(deptDiff)})
                </span>
                </>
              )}
      ...
      {childGroup.users.map((user, deptRowIndex) => (
        <DraggableRow
          ...
          daysArray={daysArray}
          rotaMap={rotaMap}
          plannedRotaMap={plannedRotaMap}
          employeeTotalDuration={employeeTotalDuration}
          employeePlannedDuration={employeePlannedDuration}
        />
      ))}
    </>
  );
...
const DraggableHierarchyBlock: React.FC<DraggableHierarchyBlockProps> = ({
  ...
  const deptTotalMins = departmentTotalDuration[hierarchy.parentGroup.department._id] || 0;
  const deptPlannedMins = departmentPlannedDuration[hierarchy.parentGroup.department._id] || 0;
  const deptDiff = deptTotalMins - deptPlannedMins;
    ...
    >
      ...
      >
        <td className="sticky left-0 z-20 border-r border-gray-200 bg-inherit px-4 py-2 shadow-[2px_0_0_0_rgba(0,0,0,0.04)]">
          <div className="flex items-center justify-between">
            ...
            {!isUnassigned && (
              <div className="flex items-center gap-1  py-1 text-[11px] font-bold ">
                {deptDiff !== 0 && (
                  <>
                  Total Diff:
                  <span className={deptDiff > 0 ? 'text-theme' : 'text-red-500'}>
      ...
      {hierarchy.childrenGroups.map((childGroup, childIndex) => (
        <DraggableChildGroup
          ...
          departmentPlannedDuration={departmentPlannedDuration}
          moveRow={moveRow}
        />
      ))}
    </tbody>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function CompanyPlannedVsActualRotaReport() {
  const { id: companyId } = useParams();
  const [currentDate, setCurrentDate] = useState(moment());
  const [users, setUsers] = useState<User[]>([]);
  const [departments, setDepartments] = useState<Department[]>([]);
  const [rotas, setRotas] = useState<any[]>([]);
  const [plannedRotas, setPlannedRotas] = useState<any[]>([]);
  ...
  const fetchUsersAndRotas = useCallback(async () => {
    ...
    try {
      ...
      const plannedRotaRes = await axiosInstance.get(
        `/planned-rota?companyId=${companyId}&startDate=${startDate}&endDate=${endDate}&limit=all`
      );
      setRotas(rotaRes.data?.data?.result || rotaRes.data?.data || []);
      setPlannedRotas(plannedRotaRes.data?.data?.result || plannedRotaRes.data?.data || []);
    } catch (err: any) {
  ...
  const { 
    ...
    const empPTotals: Record<string, Record<string, number>> = {};
    const deptPTotals: Record<string, number> = {};

    rotas.forEach((rota) => {
      const empId = typeof rota.employeeId === 'object' ? rota.employeeId._id : rota.employeeId;
      const deptId = typeof rota.departmentId === 'object' ? rota.departmentId._id : rota.departmentId;
      ...
      // Treat unpublished (pending) actual rotas as 0 minutes
      const mins = rota.status === 'pending' 
        ? 0 
        : calculateDurationMinutes(rota.startTime, rota.endTime, rota.leaveType);
    ...
    plannedRotas.forEach((rota) => {
      const empId = typeof rota.employeeId === 'object' ? rota.employeeId._id : rota.employeeId;
      const deptId = typeof rota.departmentId === 'object' ? rota.departmentId._id : rota.departmentId;
      const dateKey = moment.utc(rota.startDate).format('YYYY-MM-DD');

      if (!pMap[empId]) pMap[empId] = {};
      if (!pMap[empId][dateKey]) pMap[empId][dateKey] = [];
      pMap[empId][dateKey].push(rota);

      // Treat unpublished (pending) planned rotas as 0 minutes
      const mins = rota.status === 'pending' 
        ? 0 
        : calculateDurationMinutes(rota.startTime, rota.endTime, rota.leaveType);

      if (!empPTotals[deptId]) empPTotals[deptId] = {};
      empPTotals[deptId][empId] = (empPTotals[deptId][empId] || 0) + mins;
      deptPTotals[deptId] = (deptPTotals[deptId] || 0) + mins;
    });
    ...
    return {
      ...
      employeePlannedDuration: empPTotals,
      departmentPlannedDuration: deptPTotals
    };
  }, [rotas, plannedRotas]);
      ...
      <div className="flex h-full w-full max-w-[100vw] flex-col overflow-hidden rounded-md bg-white p-4 text-black shadow-sm">
        ...
        <div className="mb-4 flex flex-none flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <h1 className="text-lg font-bold">Planned vs. Actual Rota Report</h1>
          </div>
        ...
        <div className="mb-4 font-semibold text-gray-500 text-sm"> 
          <span className="text-black">Report Formula:</span> Actual Rota - Planned Rota = Difference
        </div>
