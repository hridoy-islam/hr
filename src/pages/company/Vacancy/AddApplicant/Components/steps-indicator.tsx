...
export function StepsIndicator({
  ...
  const progressPercentage =
     steps.length > 1 
      ? (completedSteps.length / (steps.length - 1)) * 75
      : 0;

  return (
    <div className="mb-10 pt-4 w-full relative ">
      {/* Progress bar background */}
      <div className="absolute top-10 left-[12%] right-[12%] h-1 bg-gray-200 rounded-full z-0" />

      {/* Progress bar filled */}
      <div
        className="absolute top-10 left-[12%] right-[10%] h-1 bg-blue-600 rounded-full z-10 transition-all duration-300"
