...
import {
  ArrowLeft,
  Calendar, // Using Calendar icon for RTW dates
  Upload,
  FileText,
  X,
  AlertCircle
...
interface ComplianceRow {
  _id: string; // User ID
  rtwRecordId?: string; // Document ID (optional if missing)
  email: string;
  firstName: string;
  lastName: string;
...
const RtwExpiryPage = () => {
  ...
  const [rtwCheckInterval, setRtwCheckInterval] = useState<number>(0);
  ...
  const fetchScheduleSettings = async () => {
    ...
    try {
      ...
      if (result && result.length > 0) {
        ...
        setRtwCheckInterval(result[0].rtwCheckDate || 0);
      }
    } catch (err) {
  ...
  const getComplianceStatus = (dateString: string | null) => {
    ...
    if (now.isAfter(checkDate, 'day')) {
      return 'expired';
    } else if (rtwCheckInterval > 0 && diffDays <= rtwCheckInterval) {
  ...
  const fetchEmployees = async () => {
    ...
    setLoading(true);
    try {
      // Endpoint adapted for RTW: /schedule-status/{companyId}/rtw
      const response = await axiosInstance.get(
        `/schedule-status/${companyId}/rtw`
      );
      ...
      const mappedData: ComplianceRow[] = rawData.map((item: any) => {
        ...
        return {
          _id: userObj._id,
          rtwRecordId: item._id !== userObj._id ? item._id : undefined,
      ...
      console.error('Failed to fetch RTW list:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateClick = (
    ...
    // If missing, redirect to profile (activates RTW tab logic)
    if (!employee.rtwRecordId || currentStatus === 'missing') {
      navigate(`/company/employee/${employee._id}`, {
        state: { activeTab: 'rightToWork' } // Assuming 'rtw' is the tab key in profile
      });
      return;
    }
  ...
  const handleSubmitUpdate = async () => {
    ...
    try {
      // Using the RTW specific endpoint
      const url = `/hr/right-to-work/${selectedEmployee.rtwRecordId}`;

      const payload = {
        updatedBy: user._id,
        title: 'RTW Check Updated',
      ...
      toast({
        title: 'Success',
        description: 'RTW check updated successfully',
