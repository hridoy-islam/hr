...
import {
  ArrowLeft,
  Calendar, // Changed from ShieldCheck to Calendar for Visa context
  Upload,
  FileText,
  X,
  AlertCircle
...
interface ComplianceRow {
  _id: string; // User ID
  visaRecordId?: string; // Document ID (optional if missing)
  email: string;
  firstName: string;
  lastName: string;
...
const VisaExpiryPage = () => {
  ...
  const fetchScheduleSettings = async () => {
    ...
    try {
      ...
      if (result && result.length > 0) {
        // Use visaCheckDate from settings
  ...
  const fetchEmployees = async () => {
    ...
    setLoading(true);
    try {
      // Endpoint adapted for Visa: /schedule-status/{companyId}/visa
      const response = await axiosInstance.get(
        `/schedule-status/${companyId}/visa`
      );
      ...
      const mappedData: ComplianceRow[] = rawData.map((item: any) => {
        ...
        return {
          _id: userObj._id,
          visaRecordId: item._id !== userObj._id ? item._id : undefined,
          firstName: userObj.firstName || 'Unknown',
          lastName: userObj.lastName || '',
          email: userObj.email || '',
          visaExpiry: item.expiryDate || null, 
          visaStartDate: item.startDate || null,
        };
      });

      setEmployees(mappedData);
    } catch (error) {
      console.error('Failed to fetch Visa list:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateClick = (
    ...
    // If missing, redirect to profile (activates Visa tab logic)
    if (!employee.visaRecordId || currentStatus === 'missing') {
      navigate(`/company/employee/${employee._id}`, {
        state: { activeTab: 'visa' }
      });
      return;
    }
    ...
    setNewStartDate(
      employee.visaStartDate ? new Date(employee.visaStartDate) : null
    );
    setNewExpiryDate(
      employee.visaExpiry ? new Date(employee.visaExpiry) : null
    );
  ...
  const handleSubmitUpdate = async () => {
    ...
    try {
      const url = `/visa/${selectedEmployee.visaRecordId}`;

      const payload = {
        updatedBy: user._id,
        title: 'Visa Renewed/Updated',
        document: uploadedFileUrl,
        startDate: moment(newStartDate).toISOString(),
        expiryDate: moment(newExpiryDate).toISOString(),
        employeeId: selectedEmployee._id,
        // date: new Date().toISOString() // Optional based on backend requirement for logs
      };
      ...
      toast({
        title: 'Success',
        description: 'Visa details updated successfully',
      ...
      toast({
        title: 'Error',
        description: err.response?.data?.message || 'Failed to update Visa',
        variant: 'destructive'
      });
    } finally {
              ...
              <Table>
                ...
                <TableBody>
                  {employees.length === 0 ? (
                    ...
                    employees.map((emp) => {
                      const status = getComplianceStatus(emp.visaExpiry);
          ...
          <div className="space-y-4 py-4">
            ...
            {selectedEmployee?.visaExpiry && (
              <div className="flex items-start gap-2 rounded-md bg-blue-50 p-3 text-sm text-blue-700">
                ...
                <p>
                  Current Visa expires on{' '}
                  <span className="font-semibold">
                    {moment(selectedEmployee.visaExpiry).format('DD MMM YYYY')}
                  </span>
                  .
                </p>
              </div>
