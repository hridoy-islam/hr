...
interface AttendanceUser {
  ...
  userId: {
    ...
    departmentId?: {
      ...
      name?: string; // fallback for PDF generation
    };
  };
...
export default function AttendanceReport() {
  ...
  // --------------------------------------------------------
  // FIXED: Calculate duration handling 'HH:mm' string format
  // --------------------------------------------------------
  const calculateDuration = (clockIn: string, clockOut: string): number => {
    if (!clockIn || !clockOut || clockIn === '-' || clockOut === '-') return 0;

    // 1. Try parsing "HH:mm" (e.g. "01:12")
    let start = moment(clockIn, 'HH:mm');
    let end = moment(clockOut, 'HH:mm');

    // 2. If invalid, try "HH:mm:ss"
    if (!start.isValid() || !end.isValid()) {
      start = moment(clockIn, 'HH:mm:ss');
      end = moment(clockOut, 'HH:mm:ss');
    }

    // 3. Last resort: ISO format
    if (!start.isValid() || !end.isValid()) {
      start = moment(clockIn);
      end = moment(clockOut);
    }

    // If still invalid, return 0 to prevent NaN/"Invalid date" errors
    if (!start.isValid() || !end.isValid()) {
      return 0;
    }

    // Handle overnight shifts (if End is before Start, add 1 day)
    if (end.isBefore(start)) {
      end.add(1, 'day');
    }

    const durationHours = moment.duration(end.diff(start)).asHours();
    return isNaN(durationHours) ? 0 : durationHours;
  };
  ...
  useEffect(() => {
    ...
    console.log('User history records:', userHistory.map(record => ({
      ...
      isValid: moment(record.clockIn, ["HH:mm", "HH:mm:ss"]).isValid()
    })));
  }, [userHistory]);
  ...
  const generateReport = async () => {
    ...
    setIsLoading(true);
    try {
      const data = await getAttendanceReport(fromDate, toDate, user?._id);
      ...
      data.forEach((record: any) => {
        ...
        if (record.clockIn && record.clockOut) {
          ...
          // FIXED: Late detection using flexible parsing
          let clockInTime = moment(record.clockIn, 'HH:mm');
          if (!clockInTime.isValid()) clockInTime = moment(record.clockIn, 'HH:mm:ss');
          
          if (clockInTime.isValid() && clockInTime.isAfter(moment('09:30:00', 'HH:mm:ss'))) {
            user.lateCount += 1;
          }
        }
      });
  ...
  const viewUserHistory = async (user: AttendanceUser) => {
    ...
    try {
      ...
      // FIXED: Calculate duration specifically for the history table
      const processedHistory = history.map((record: any) => {
        let formattedDuration = '-';

        if (record.clockIn && record.clockOut && record.clockIn !== '-' && record.clockOut !== '-') {
          const hours = calculateDuration(record.clockIn, record.clockOut);
          // Convert decimal hours to HH:mm:ss string safely
          // If hours is 0, it outputs "00:00:00", never "Invalid date"
          formattedDuration = moment.utc(moment.duration(hours, 'hours').asMilliseconds()).format('HH:mm:ss');
        }

        return {
          ...record,
          duration: record.duration || formattedDuration
        };
      });
                ...
                <TableBody>
                  {userHistory.map((record) => (
                    <TableRow key={record._id}>
                      <TableCell>
                        {/* Display date cleanly if possible, otherwise raw string */}
                ...
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                  <div className="rounded-md bg-white p-3 shadow-sm">
                    ...
                    <div className="text-lg font-semibold">
                      {
                        userHistory.filter((r) => r.clockIn && r.clockOut && r.clockIn !== '-' && r.clockOut !== '-')
                          .length
                      }
                    </div>
                  </div>
