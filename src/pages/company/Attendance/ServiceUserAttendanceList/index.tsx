  ...
  const fetchAttendance = async (page: number, limit: number) => {
    ...
    try {
      const params = {
        ...
        page,
        limit,
        userType: 'service_user', // Crucial: Only fetch visitors
        fromDate: startDate
      ...
      <Table>
        ...
        <TableBody>
          {data.map((record: any) => {
            ...
            const serviceUserName = record.serviceUserId?.name || 'Unknown';
            const room = record.serviceUserId?.room || '__';
...
export default ServiceUserAttendancePage;
