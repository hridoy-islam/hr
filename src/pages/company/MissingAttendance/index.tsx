...
export default function MissingAttendancePage() {
  ...
  const fetchRotas = async (isReset = false) => {
    ...
    try {
      ...
      toast({
        ...
        title: 'Fetch Error',
        description:
          err?.response?.data?.message || 'Failed to fetch missed shifts'
      });
      setRotas([]);
    } finally {
      setIsLoading(false);
  ...
  const handleSubmit = async (rotaId: string, isReconciled: boolean) => {
    ...
    try {
      const payloadClockInDate = moment(finalStartDate)
        .startOf('day')
        .toISOString();
      const payloadClockOutDate = moment(finalEndDate)
        .startOf('day')
        .toISOString();
      const payloadClockIn = moment(
        `${finalStartDate} ${finalStartTime}`,
        'YYYY-MM-DD HH:mm'
      ).toISOString();
      ...
      const employeeIdToSubmit =
        typeof selectedRota.employeeId === 'object'
  ...
  return (
    <div className="flex min-h-[70vh] w-full flex-col gap-6 rounded-md bg-white p-6 shadow-sm">
      {/* ── Header & Filters ── */}
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="w-64 min-w-[200px]">
            ...
            <Select
              ...
              onChange={(option) =>
                setSelectedEmployee(option as EmployeeOption)
              }
          ...
          <div className="flex items-end gap-2 self-end pb-[1px]">
            <Button
              ...
              className="h-[38px] gap-2 bg-blue-600 hover:bg-blue-700"
            >
      ...
      <div className="flex-1">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center rounded-lg border border-gray-300 bg-gray-50 py-12 text-center">
              ...
              <TableBody>
                {rotas.map((rota) => {
                  ...
                  const departmentName =
                    rota.departmentId?.departmentName || '--';
                  const fullName = employeeObj
                    ? `${employeeObj.firstName || ''} ${employeeObj.lastName || employeeObj.name || ''}`.trim()
                    : 'Unknown';
                      ...
                      <TableCell className="font-mono text-sm">
                        {isEditing ? (
                          <Input
                            ...
                            onBlur={(e) =>
                              handleTimeBlur('startTime', e.target.value)
                            }
                            placeholder="HH:mm"
                      ...
                      <TableCell className="font-mono text-sm">
                        {isEditing ? (
                          <Input
                            ...
                            onBlur={(e) =>
                              handleTimeBlur('endTime', e.target.value)
                            }
                            placeholder="HH:mm"
                            className="h-8 w-20 font-mono text-sm"
                            maxLength={5}
