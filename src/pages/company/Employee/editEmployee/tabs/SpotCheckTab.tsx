...
function SpotCheckTab() {
  ...
  const submitCompletion = async () => {
    ...
    if (!inputDate || uploadedFiles.length === 0 || !spotCheckId || !id) return;
    setIsSubmitting(true);
  ...
  const handleSubmitEditLog = async () => {
    if (!spotCheckId || !editingLog || !editingLog._id) return;
  ...
  const renderFileInput = (isOptional: boolean = false) => (
    <div className="space-y-3 pt-2">
      <Label className="text-sm font-medium text-gray-700">
        Proof Document(s) <span className={isOptional ? "text-gray-400 font-normal" : "text-red-500"}>
          {isOptional ? "(Optional)" : "*"}
        </span>
      </Label>
    ...
    <div>
            ...
            <div className="space-y-6">
              ...
              {!showCompleteButton && completionDate && (
                <div className="rounded-md bg-green-50 p-3 text-sm text-green-700 flex items-start gap-2">
                  ...
                  <p>Last check completed on <strong>{moment(completionDate).format('DD MMM YYYY')}</strong></p>
                </div>
              )}
                ...
                <div className={cn(
                  ...
                  <p>
                    {complianceStatus === 'overdue' 
                      ? 'Spot check is overdue.' 
                      : 'Spot check is due soon.'}
                  </p>
                </div>
              )}
      ...
      <Dialog open={showScheduleModal} onOpenChange={setShowScheduleModal}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{scheduledDate ? 'Update Schedule' : 'Schedule Spot Check'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            {/* Added: Optional Document Upload for Schedule */}
            {renderFileInput(true)}

          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowScheduleModal(false)} disabled={isSubmitting || isUploading}>Cancel</Button>
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea 
                placeholder="Enter details about the check..." 
                value={inputNote}
                onChange={(e) => setInputNote(e.target.value)}
                rows={3}
              />
            ...
            {/* Required Document Upload for Completion */}
            {renderFileInput(false)}

          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowCompleteModal(false)} disabled={isSubmitting || isUploading}>Cancel</Button>
            <Button 
