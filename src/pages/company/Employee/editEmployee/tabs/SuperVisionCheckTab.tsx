...
function SupervisionTab() {
  ...
  const [editCompletionDate, setEditCompletionDate] = useState<Date | null>(
    null
  );
  ...
  const handleOpenSchedule = () => {
    setInputDate(
      scheduledDate
        ? new Date(
            moment.utc(scheduledDate).year(),
            moment.utc(scheduledDate).month(),
            moment.utc(scheduledDate).date()
          )
        : null
    );
    setInputNote('');
  ...
  const submitSchedule = async () => {
    ...
    try {
      ...
      toast({
        title: 'Supervision scheduled successfully',
  ...
  const submitCompletion = async () => {
    ...
    if (!inputDate || uploadedFiles.length === 0 || !supervisionId || !id)
      return;
    setIsSubmitting(true);
    ...
    try {
      ...
      await fetchSupervisionData();
      toast({
        title: 'Supervision marked as completed',
        className: 'bg-theme text-white'
      });
      ...
      toast({
        title: error.response?.data?.message || 'Failed to complete',
        className: 'bg-destructive text-white'
      });
    } finally {
      setIsSubmitting(false);
  ...
  const openEditLogModal = (entry: LogEntry) => {
    ...
    setEditScheduledDate(
      entry.scheduledDate
        ? new Date(
            moment.utc(entry.scheduledDate).year(),
            moment.utc(entry.scheduledDate).month(),
            moment.utc(entry.scheduledDate).date()
          )
        : null
    );
    setEditCompletionDate(
      entry.completionDate
        ? new Date(
            moment.utc(entry.completionDate).year(),
            moment.utc(entry.completionDate).month(),
            moment.utc(entry.completionDate).date()
          )
        : null
    );
  ...
  const handleToggleClose = async () => {
    ...
    try {
      ...
      toast({
        title: isClosed
          ? 'Supervision reopened successfully!'
          : 'Supervision closed successfully!',
        className: 'bg-theme text-white'
      });
  ...
  const handleSubmitEditLog = async () => {
    ...
    try {
      await axiosInstance.patch(
        `/supervision/${supervisionId}/logs/${editingLog._id}`,
        {
          document: finalDocuments,
          scheduledDate: editScheduledDate
            ? new Date(
                Date.UTC(
                  editScheduledDate.getFullYear(),
                  editScheduledDate.getMonth(),
                  editScheduledDate.getDate()
                )
              ).toISOString()
            : null,
          completionDate: editCompletionDate
            ? new Date(
                Date.UTC(
                  editCompletionDate.getFullYear(),
                  editCompletionDate.getMonth(),
                  editCompletionDate.getDate()
                )
              ).toISOString()
            : null,
          note: editNote
            ...
            >
              <div className="flex items-center gap-2 overflow-hidden">
                ...
                <button
                  ...
                  className="truncate text-left text-xs font-medium text-green-700 hover:underline"
                  title={file.name}
                >
                  {file.name}
                </button>
    ...
    <div>
            ...
            <div className="space-y-6">
              <div className="space-y-1">
                ...
                <div className="text-lg font-semibold text-gray-900">
                  {scheduledDate
                    ? moment(scheduledDate).format('DD MMMM YYYY')
                    : 'Not Set'}
                </div>
              </div>
              ...
              {savedNote && (
                <div className="space-y-1">
                  ...
                  <div className="whitespace-pre-wrap rounded-md border border-gray-100 bg-gray-50 p-3 text-sm text-gray-700">
                    {savedNote}
                  </div>
                </div>
              )}
              ...
              {completionDate && (
                <div className="flex items-start gap-2 rounded-md bg-green-50 p-3 text-sm text-green-700">
                  ...
                  <p>
                    Last supervision completed on{' '}
                    <strong>
                      {moment(completionDate).format('DD MMM YYYY')}
                    </strong>
                  </p>
                </div>
              )}
              ...
              <div className="space-y-3 border-t border-gray-100 pt-6">
                ...
                >
                  ...
                  {scheduledDate
                    ? 'Update Supervision Date'
                    : 'Create First Supervision Date'}
                </Button>
      ...
      <Dialog open={showScheduleModal} onOpenChange={setShowScheduleModal}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {scheduledDate ? 'Update Schedule' : 'Schedule Supervision'}
            </DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={!inputDate || isSubmitting || isUploading}
            >
