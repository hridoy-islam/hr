...
const createPolicySchema = z
  .object({
    title: z
      ...
      .min(3, { message: 'Policy title must be at least 3 characters long' })
      .max(100, { message: 'Policy title cannot exceed 100 characters' }),
    startDate: z
      .date({ invalid_type_error: 'Please select a valid start date' })
      .optional()
      .nullable(),
    expiryDate: z
      .date({ invalid_type_error: 'Please select a valid expiry date' })
      .optional()
      .nullable(),
    updatedBy: z
...
export default function CompanyPolicyPage() {
  ...
  const [policyCheckInterval, setPolicyCheckInterval] = useState(30);
  ...
  // Dynamic status calculation per policy
  const getStatusBadge = (expiryDateString?: string) => {
    // If there is no expiry date, default to Active status
    if (!expiryDateString) {
      return (
  ...
  // Fetch Policies List
  const fetchPolicies = async (page: number, limit: number, search = '') => {
    try {
      if (initialLoading) setInitialLoading(true);

      const response = await axiosInstance.get(`/company-policy`, {
        params: {
          page,
          limit,
          companyId: id,
      ...
      setPolicies(response.data.data.result || response.data.data);
  ...
  useEffect(() => {
    fetchPolicies(currentPage, entriesPerPage);
  ...
  const handleSearch = () => {
    fetchPolicies(currentPage, entriesPerPage, searchTerm);
  };
  ...
  const handleCreatePolicy = async () => {
    ...
    // Using `undefined` if date is null avoids passing nulls to schemas that expect omission for optional fields
    const rawPayload = {
      title: policyTitle,
      startDate: startDate ? normalizeDate(startDate) : undefined,
      expiryDate: expiryDate ? normalizeDate(expiryDate) : undefined,
      updatedBy: user?._id || 'unknown',
      companyId: id || '',
      document: uploadedFiles.map((file) => file.url) // Matches the Mongoose 'document' field
    };

    const validation = createPolicySchema.safeParse(rawPayload);
    ...
    try {
      ...
      toast({
        title: error.response?.data?.message || 'Failed to create policy',
  ...
  const handleDeletePolicy = async (policyId: string) => {
    try {
      ...
      toast({
        title: error.response?.data?.message || 'Failed to delete policy',
        className: 'bg-red-500 border-none text-white'
      });
    }
  };
            ...
            <TableBody>
              {policies.map((policy) => (
                <TableRow key={policy._id}>
                  ...
                  <TableCell>
                    {policy.startDate
                      ? moment(policy.startDate).format('DD MMM, YYYY')
                      : '-'}
                  </TableCell>
                  <TableCell>
                    {policy.expiryDate
                      ? moment(policy.expiryDate).format('DD MMM, YYYY')
                      : '-'}
                  </TableCell>
                  <TableCell>
                    {getStatusBadge(policy.expiryDate)}
                  </TableCell>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              permanently delete this policy and remove its data
                              from our servers.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
        ...
        <DialogContent className="flex max-h-[95vh] max-w-2xl flex-col overflow-y-auto p-4 sm:p-6 md:p-8">
          ...
          <div className="mt-2 flex-1 space-y-6 overflow-y-auto">
            ...
            <div className="space-y-1">
              <label className="text-sm font-semibold">Policy Title</label>
              <Input
          ...
          <DialogFooter className="mt-6 flex-col gap-2 border-t border-gray-100 pt-4 sm:flex-row sm:gap-0">
            ...
            >
              {isSubmitting ? <BlinkingDots size="small" /> : 'Create Policy'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
