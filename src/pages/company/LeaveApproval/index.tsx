...
const CompanyLeaveApprovalPage: React.FC = () => {
  ...
  // Pending form data captured when user clicks Approve, before confirmation
  const [pendingApproveData, setPendingApproveData] = useState<LeaveProcessFormValues | null>(null);
  // Reject an already-approved leave
  const [showRejectApprovedModal, setShowRejectApprovedModal] = useState(false);
  // Delete a rejected leave
  const [showDeleteRejectedModal, setShowDeleteRejectedModal] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const [isRejecting, setIsRejecting] = useState(false);
  const [isUpdating, setIsUpdating] = useState(false);
  const [showUpdateConfirmModal, setShowUpdateConfirmModal] = useState(false);
  const [pendingUpdateData, setPendingUpdateData] = useState<LeaveProcessFormValues | null>(null);
  ...
  const [approvalEmployeeHoursPerDay, setApprovalEmployeeHoursPerDay] = useState<number>(8);
  ...
  const [selectedYear, setSelectedYear] = useState<string>(getCurrentHolidayYear());
  ...
  const [createSelectedEmployee, setCreateSelectedEmployee] = useState<EmployeeOption | null>(null);
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadPromises = files.map(async (file) => {
        ...
        const targetUserId =
          selectedLeave?.userId?._id || createSelectedEmployee?.value || user?._id;
  ...
  const handleRemoveFile = (indexToRemove: number) => {
    setUploadedFiles((prev) => {
      ...
      if (isSheetOpen && selectedLeave) {
        setSelectedLeave((curr) =>
          curr ? { ...curr, documents: updated.map((f) => f.url) } : curr
        );
      }
      return updated;
    });
  ...
  // FETCH HOURS PER DAY FOR CREATE MODE
  useEffect(() => {
    ...
    const fetchEmployeeAllowance = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          record = data.find((item: any) => item.year === selectedYear) || data[0];
        } else {
          record = data;
        }
        ...
        if (createStartDate && createEndDate && selectedType) {
          calculateLeaveData(createStartDate, createEndDate, selectedType, fetchedHours);
        }
      } catch (err) {
  ...
  // FETCH HOURS PER DAY FOR APPROVAL MODE
  useEffect(() => {
    ...
    const fetchApprovalEmployeeAllowance = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          record =
            data.find((item: any) => item.year === selectedLeave.holidayYear) || data[0];
        } else {
          record = data;
        }
  ...
  const calculateLeaveData = (
    ...
    if (start && end) {
      ...
      if (isHoliday) {
        ...
        if (hoursOverride > 0) {
          setCalculatedDays(String(parseFloat((totalHrs / hoursOverride).toFixed(2))));
        } else {
  ...
  const handleDaysChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (createStartDate) {
      ...
      if (selectedType === 'holiday') {
        ...
        let current = new Date(createStartDate);
        while (current <= newEndDate) {
          daysArr.push({ leaveDate: new Date(current), leaveType: 'unpaid', duration: 0 });
          current.setDate(current.getDate() + 1);
        }
        setLeaveDays(daysArr);
      }
  ...
  const handleSubmitRequest = async () => {
    if (!createSelectedEmployee) {
      setFormErrors((prev) => ({ ...prev, employee: 'Please select an employee' }));
      return;
    }
    ...
    const formData = {
      ...
      totalDays: calculatedDays === '' ? '' : parseFloat(String(calculatedDays)),
      totalHours:
  ...
  // --- APPROVE / UPDATE / REJECT / DELETE ACTIONS ---
    ...
    try {
      ...
      if ((action === 'approved' || action === 'update') && formData) {
        ...
        payload.leaveDays = editLeaveDays.map((day) => ({
          ...
          duration:
            selectedLeave?.holidayType === 'holiday' ||
            selectedLeave?.holidayType === 'sick'
              ? isNaN(parseFloat(String(day.duration)))
                ? 0
                : parseFloat(String(day.duration))
              : 0
  ...
  // Reject an already-approved leave (patch status → rejected)
  const handleRejectApproved = async () => {
    if (!selectedLeave) return;
    setIsRejecting(true);
    try {
      await axiosInstance.patch(`/hr/leave/${selectedLeave._id}`, {
        status: 'rejected',
        actionUserId: user?._id
      });
      toast({ title: 'Leave request has been rejected successfully.' });
  ...
  // Update an already-approved leave's details
  const handleUpdateApproved = async (formData: LeaveProcessFormValues) => {
    if (!selectedLeave) return;
    setIsUpdating(true);
    try {
      const payload: any = {
      ...
      await axiosInstance.patch(`/hr/leave/${selectedLeave._id}`, payload);
      toast({ title: 'Leave details updated successfully.' });
      ...
      toast({
        title: 'Failed to update leave details',
        description: err.response?.data?.message || 'Check console for details',
        variant: 'destructive'
      });
    } finally {
  ...
  const handleDeleteRejected = async () => {
    ...
    setIsDeleting(true);
    try {
      await axiosInstance.delete(`/hr/leave/${selectedLeave._id}`);
  ...
  const openApprovalSheet = (request: LeaveRequest) => {
    ...
    if (request.documents && request.documents.length > 0) {
      setUploadedFiles(
        request.documents.map((url) => ({ name: getFileNameFromUrl(url), url }))
      );
    } else {
      setUploadedFiles([]);
    }
  ...
  const formatDate = (dateString: string) =>
    moment(dateString).startOf('day').format('DD MMM YYYY');

  const getStatusBadgeClass = (status: string) => {
    if (status === 'approved') return 'bg-green-500 text-white hover:bg-green-500';
  ...
  const LeaveTooltipContent = ({ request }: { request: LeaveRequest }) => {
    ...
    const allowanceStatsList = allowance
      ? [
          { label: 'Carry Forward From Last Year', value: allowance.carryForward, color: 'text-gray-900', labelClass: 'text-gray-500' },
          { label: 'Present Year Holiday Entitlement', value: allowance.holidayEntitlement, color: 'text-gray-900', labelClass: 'text-gray-500' },
          { label: 'Opening This Year', value: allowance.holidayAllowance, color: 'text-red-700', labelClass: 'text-gray-500' },
          { label: 'Holiday Accrued', value: allowance.holidayAccured, color: 'text-gray-900', labelClass: 'text-gray-500' },
          { label: 'Taken', value: allowance.usedHours, color: 'text-green-600', labelClass: 'text-gray-500' },
          { label: 'Booked', value: allowance.bookedHours, color: 'text-orange-600', labelClass: 'text-gray-500' },
          { label: 'Requested', value: allowance.requestedHours, color: 'text-yellow-500', labelClass: 'text-gray-500' },
          { label: 'Balance Remaining', value: allowance.remainingHours, color: 'text-red-700', labelClass: 'font-bold text-gray-900' },
          { label: 'Unpaid Leave Taken', value: allowance.unpaidLeaveTaken, color: 'text-cyan-500', labelClass: 'text-gray-500' },
          { label: 'Unpaid Booked', value: allowance.unpaidBookedHours, color: 'text-blue-600', labelClass: 'text-gray-500' },
          { label: 'Unpaid Requested', value: allowance.unpaidLeaveRequest, color: 'text-blue-600', labelClass: 'text-gray-500' }
        ]
      : [];
        ...
        <div className="px-3 pb-2 text-[13px]">
          {loading ? (
            ...
            <div className="flex flex-col">
              {allowanceStatsList.map(({ label, value, color, labelClass }, index) => (
                <div
                  key={label}
                  className={`flex items-center justify-between py-[10px] ${
                    index !== allowanceStatsList.length - 1 ? 'border-b border-gray-200' : ''
                  }`}
                >
                  <span className={labelClass}>{label}</span>
                  <span className={`font-semibold ${color}`}>{value.toFixed(2)} h</span>
                </div>
              ))}
            </div>
          ) : (
        ...
        <Card className="shadow-sm">
                    ...
                    <div className="space-y-2 max-h-screen">
                      <div className="grid grid-cols-2 items-center gap-2">
                        ...
                        <div className="-mt-2">
                          ...
                          >
                            <SelectTrigger
                              id="holiday-year"
                              className={`h-[38px] ${formErrors.holidayYear ? 'border-red-500' : ''}`}
                            >
                          ...
                          {formErrors.holidayYear && (
                            <p className="mt-1 text-xs text-red-500">{formErrors.holidayYear}</p>
                          )}
                        </div>

                        <div>
                          <Label htmlFor="type" className="mb-1 block">Holiday Type</Label>
                          <ReactSelect
                            inputId="type"
                            options={leaveTypeOptions}
                            value={leaveTypeOptions.find((opt) => opt.value === selectedType) || null}
                            onChange={(option) => {
                              const newType = option?.value || '';
                              setSelectedType(newType);
                              clearError('holidayType');
                              calculateLeaveData(createStartDate, createEndDate, newType);
                            }}
                            placeholder="Select type"
                            styles={{
                              control: (base) => ({
                                ...base,
                                borderColor: formErrors.holidayType ? '#ef4444' : base.borderColor,
                                '&:hover': {
                                  borderColor: formErrors.holidayType ? '#ef4444' : base.borderColor
                                }
                              })
                          ...
                          {formErrors.holidayType && (
                            <p className="mt-1 text-xs text-red-500">{formErrors.holidayType}</p>
                          )}
                        </div>
                        ...
                        <div className="flex flex-col gap-1 py-2">
                          ...
                          <DatePicker
                            ...
                            onChange={(dates) => {
                              ...
                              calculateLeaveData(safeStart, safeEnd, selectedType);
                            }}
                            isClearable
                          ...
                          {formErrors.dateRange && (
                            <p className="mt-1 text-xs text-red-500">{formErrors.dateRange}</p>
                          )}
                        </div>
                      </div>
                      ...
                      {selectedType && (
                        <div className="space-y-1">
                          {selectedType === 'holiday' && leaveDays.length > 0 && (
                            <>
                                ...
                                {leaveDays.map((day, idx) => (
                                  ...
                                  >
                                    ...
                                    <Input
                                      ...
                                      value={day.duration}
                                      onChange={(e) => handleDayDurationChange(idx, e.target.value)}
                                    />
                                  </div>
                                ))}
                              </div>
                          ...
                          <div className="grid grid-cols-2 mt-4 gap-4">
                            <div className="space-y-1">
                              <Label htmlFor="duration-days">
                                {selectedType === 'holiday' ? 'Holiday Duration (Days)' : 'Duration (Days)'}
                              </Label>
                              <Input
                                id="duration-days"
                                type="number"
                              ...
                              {formErrors.totalDays && (
                                <p className="text-xs text-red-500">{formErrors.totalDays}</p>
                              )}
                            </div>
                            {selectedType === 'holiday' && (
                              <div className="space-y-1">
                                ...
                                {formErrors.totalHours && (
                                  <p className="text-xs text-red-500">{formErrors.totalHours}</p>
                                )}
                              </div>
                            )}
                          </div>
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-1 gap-4 pb-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="space-y-1">
                <label className="text-xs font-semibold uppercase tracking-wide">Employee</label>
                <ReactSelect
                  options={employeeOptions}
              ...
              <div className="w-full space-y-1">
                <label className="text-xs font-semibold uppercase tracking-wide">Date Range (DD-MM-YYYY)</label>
                <DatePicker
                  selectsRange
                  startDate={startDate}
                  endDate={endDate}
              ...
              <div className="mt-6 flex items-center justify-start gap-2">
                ...
                <Button variant="outline" onClick={handleReset} className="h-10 gap-2">
                      ...
                      {leaves.length === 0 ? (
                        ...
                        leaves.map((request) => (
                          <TableRow key={request._id} className="hover:bg-gray-50">
                            ...
                            <TableCell className="text-sm">{formatDate(request.startDate)}</TableCell>
                            <TableCell className="text-sm">{formatDate(request.endDate)}</TableCell>
                            <TableCell>{request.totalDays}d ({request.totalHours}h)</TableCell>
                            <TableCell className="max-w-xs truncate text-sm">{request.reason}</TableCell>
                            <TableCell>
                            ...
                            <TableCell className="text-right">
                              <Button size="sm" onClick={() => openApprovalSheet(request)}>Check</Button>
                            </TableCell>
                          </TableRow>
                        ))
                      )}
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div className="text-md flex items-center justify-between pb-4 font-semibold">
                        <div className="flex flex-row items-center gap-2">
                          <span>
                            Staff: {selectedLeave.userId?.firstName} {selectedLeave.userId?.lastName}
                          </span>
                      ...
                      {/* Daily Duration Breakdown (holiday type) */}
                      {selectedLeave.holidayType === 'holiday' && editLeaveDays.length > 0 && (
                            ...
                            {editLeaveDays.map((day: any, idx: number) => (
                              ...
                              >
                                ...
                                <Input
                                  ...
                                  value={day.duration}
                                  onChange={(e) => handleEditDayDurationChange(idx, e.target.value)}
                                  disabled={
                            ...
                            render={({ field, fieldState }) => (
                              <>
                                <Input
                                  ...
                                  {...field}
                                  value={
                                    field.value === 0 || Number.isNaN(field.value) ? '' : field.value
                                  }
                                  onChange={(e) => {
                                    ...
                                    if (newDays >= 1 && selectedLeave?.startDate) {
                                      ...
                                      if (selectedLeave.holidayType === 'holiday') {
                                        ...
                                        const daysArr = [];
                                        let current = new Date(start);
                                        const hrsPerDay = newHours / (addedDays + 1);
                                        while (current <= newEndDate) {
                                          daysArr.push({
                                            leaveDate: new Date(current),
                                            leaveType: hrsPerDay === 0 ? 'dayoff' : 'paid',
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      if (
                                        ...
                                        const hrsPerDay = Number(parsedHours) / editLeaveDays.length;
                                        setEditLeaveDays(
                                          editLeaveDays.map((day: any) => ({
                                            ...day,
                                            duration: parseFloat(hrsPerDay.toFixed(2)),
                                            leaveType: hrsPerDay === 0 ? 'dayoff' : 'paid'
                                          }))
                      ...
                      <div className="flex justify-between gap-3 border-t border-gray-100 pt-6 mt-6">
                        ...
                        {selectedLeave.status === 'pending' && (
                          <>
                            ...
                            <div className="flex flex-row items-center gap-4">
                              ...
                              {/* Approve click → open confirmation modal first */}
                              <Button
                        ...
                        {/* APPROVED: show Reject + Update buttons */}
                        {selectedLeave.status === 'approved' && (
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <Button
                                onClick={() => {
                                  ...
                                  const startDate = form.getValues('startDate') || selectedLeave.startDate;
                                  const endDate = form.getValues('endDate') || selectedLeave.endDate;

                                  if (selectedLeave?.holidayType === 'holiday' && Number(totalHours) < 1) {
                                    form.setError('totalHours', {
                                      type: 'manual',
                                      message: 'Total hours must be at least 1 for holidays'
                                    });
                                  ...
                                  const data: LeaveProcessFormValues = {
                        ...
                        {selectedLeave.status === 'rejected' && (
                          <div className="flex w-full justify-between gap-3">
                            <Button
                              ...
                              onClick={() => {
                                setIsSheetOpen(false);
                                setShowDeleteRejectedModal(true);
                              }}
                            >
                      ...
                      {selectedLeave.history && selectedLeave.history.length > 0 && (
                        <div className="mb-6 mt-8 space-y-4 rounded-lg">
                          ...
                          <div className="space-y-3">
                            {[...selectedLeave.history].reverse().map((log: any, idx: number) => (
                              <div
                                key={idx}
            ...
            {/* ── MODAL: Confirm Reject (pending leave) ── */}
            <Dialog open={showRejectConfirmModal} onOpenChange={setShowRejectConfirmModal}>
              <DialogContent>
                ...
                <DialogFooter className="mt-4">
                  ...
                  <Button onClick={() => handleActionSubmit('rejected')} variant="destructive">
                    Confirm Rejection
                  </Button>
                </DialogFooter>
              </DialogContent>
                ...
                <div className="space-y-3">
                  ...
                  <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
                    <p className="flex items-start gap-2 text-sm font-medium text-amber-800">
                      ...
                      This action will affect the rota and cannot be undone. The employee's
                      schedule will be updated immediately.
                    </p>
                  </div>
                </div>
                <DialogFooter className="mt-4">
                  ...
                  <Button
                    onClick={() => {
                      if (pendingApproveData) {
                        handleActionSubmit('approved', pendingApproveData);
                      }
                    }}
                  >
                    Confirm Approval
            ...
            {/* ── MODAL: Update an approved leave ── */}
            <Dialog open={showUpdateConfirmModal} onOpenChange={setShowUpdateConfirmModal}>
              <DialogContent>
                ...
                <div className="space-y-3">
                  <p className="text-sm text-gray-700">
                    You are about to update the approved leave details for{' '}
                    <strong>
                      {selectedLeave?.userId?.firstName} {selectedLeave?.userId?.lastName}
                    </strong>
                    .
                ...
                <DialogFooter className="mt-4">
                  ...
                  <Button
                    onClick={() => {
                      if (pendingUpdateData) handleUpdateApproved(pendingUpdateData);
                    }}
                    disabled={isUpdating}
                  >
            ...
            {/* ── MODAL: Reject an already-approved leave ── */}
            <Dialog open={showRejectApprovedModal} onOpenChange={setShowRejectApprovedModal}>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2">
                    <AlertTriangle className="h-5 w-5 text-red-500" />
                ...
                <div className="space-y-3">
                  <p className="text-sm text-gray-700">
                    You are about to reject a previously approved leave for{' '}
                    <strong>
                      {selectedLeave?.userId?.firstName} {selectedLeave?.userId?.lastName}
                    </strong>
                    .
                  ...
                  <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3">
                    <p className="flex items-start gap-2 text-sm font-medium text-red-800">
                      ...
                      This action will affect the rota and cannot be undone. The employee's
                      approved leave will be reversed and their allowance recalculated.
                    </p>
                  </div>
                </div>
                <DialogFooter className="mt-4">
            ...
            {/* ── MODAL: Delete a rejected leave ── */}
            <Dialog open={showDeleteRejectedModal} onOpenChange={setShowDeleteRejectedModal}>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle className="flex items-center gap-2">
                ...
                <div className="space-y-3">
                  <p className="text-sm text-gray-700">
                    You are about to permanently delete the rejected leave request for{' '}
                    <strong>
                      {selectedLeave?.userId?.firstName} {selectedLeave?.userId?.lastName}
                    </strong>
                    .
                  ...
                  <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3">
                    <p className="flex items-start gap-2 text-sm font-medium text-red-800">
                      <AlertTriangle className="mt-0.5 h-4 w-4 flex-shrink-0 text-red-600" />
                      This is a permanent action and cannot be undone. The leave record will be
                      removed entirely.
                    </p>
                  </div>
                </div>
