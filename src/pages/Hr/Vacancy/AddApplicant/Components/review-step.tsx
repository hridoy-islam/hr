...
const ReviewStep = ({ formData, onSubmit, onBack }) => {
  ...
  return (
    <div>
      <div className="mx-auto w-full space-y-8 rounded-xl bg-white px-6 py-10 shadow-lg">
        ...
        <div className="flex items-center gap-4">
          <img
            src={
              formData?.profilePictureUrl || 'https://via.placeholder.com/150'
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-4 text-gray-700 md:grid-cols-2">
            <p>
              Title:{' '}
              <span className="font-medium">{personalDetails.title}</span>
            </p>
            <p>
              Name:{' '}
        ...
        <section>
          ...
          <p>
            NHS Number:{' '}
            <span className="font-medium">{personalDetails.nhsNumber}</span>
          </p>
          <p>
            Passport No:{' '}
            <span className="font-medium">{personalDetails.passportNo}</span>
          </p>
          <p>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-4 text-gray-700 md:grid-cols-2">
            ...
            <p>
              Position:{' '}
              <span className="font-medium">
                {personalDetails.position || 'N/A'}
              </span>
            </p>
            <p>
              Source:{' '}
              <span className="font-medium">
                {personalDetails.source || 'N/A'}
              </span>
            </p>
            <p>
              Branch:{' '}
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-4 text-gray-700 md:grid-cols-2">
            ...
            <p>
              Home Phone:{' '}
              <span className="font-medium">{contact.homePhone || 'N/A'}</span>
            </p>
            <p>
              Other Phone:{' '}
              <span className="font-medium">{contact.otherPhone || 'N/A'}</span>
            </p>
            <p>
              Address:
              <span className="font-medium">
                {contact.address || 'N/A'}, {contact.cityOrTown || 'N/A'},{' '}
                {contact.stateOrProvince || 'N/A'}, {contact.postCode || 'N/A'},{' '}
                {contact.country || 'N/A'}
              </span>
            </p>
          </div>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-4 text-gray-700 md:grid-cols-2">
            <p>
              Gender:{' '}
              <span className="font-medium">{demography.gender || 'N/A'}</span>
            </p>
            <p>
              Marital Status:{' '}
              <span className="font-medium">
                {demography.maritalStatus || 'N/A'}
              </span>
            </p>
            <p>
              Ethnic Origin:{' '}
              <span className="font-medium">
                {demography.ethnicOrigin || 'N/A'}
              </span>
            </p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-2 text-gray-700">
            <p>
              Has Disability:{' '}
              <span className="font-medium">
                {demography.hasDisability ? 'Yes' : 'No'}
              </span>
            </p>
            <p>
              Details:{' '}
            ...
            <p>
              Needs Adjustment:{' '}
              <span className="font-medium">
                {demography.needsReasonableAdjustment ? 'Yes' : 'No'}
              </span>
            </p>
            <p>
              Adjustment Details:{' '}
        ...
        <div className="flex justify-between gap-4 pt-6">
          ...
          <Button
            onClick={onSubmit}
            className="rounded-md bg-supperagent px-4 py-2 text-white hover:bg-supperagent/90"
          >
            Submit
          </Button>
        </div>
